...
public static class CopyEditPromptBuilder
{
    ...
    /// </summary>
    public static string GetStorySystemPrompt() =>
        """
        ...
        Cut what doesn't work. If a passage catalogs names or events in sequence, repeats the same dramatic beat in parallel structure, or reads as a report of what happened rather than lived experience, you have permission to compress or remove it. Machine-generation patterns — template repetition, list-like sequences, prompt content surfacing as narrative — should be broken or cut.

        Read for rhythm. Where a sentence fights you or the prose stumbles, recast it — but preserve its content and intent.
    ...
    /// </summary>
    public static string GetDocumentSystemPrompt() =>
        """
        ...
        What you must preserve:
        - Every piece of information the document conveys. If the same information appears in two sections that serve different purposes, both belong. If two sections serve the same purpose — merge them into one. Nothing the document *says* changes, but it shouldn't say the same thing twice for the same reason.
        - The document's voice and register. A bureaucratic report should stay bureaucratic. A folk collection should stay collected. Do not normalize register across sections that are intentionally different.
        ...
        Tighten. Every word should earn its place. Look for redundant framing, bureaucratic padding that doesn't serve the document's voice, repeated information that appears in multiple sections for the same purpose, and explanations that duplicate what the document already establishes.

        Cut what doesn't work. If a section catalogs information without purpose, repeats the same content in template form, or pads the document without adding substance, you have permission to compress or remove it. Machine-generation patterns should be broken or cut.

        Read for consistency. Formatting conventions should be uniform throughout.
        ...
        IReadOnlyList<string>? motifs = null)
    {
        ...
        sections.Add($"Format: {styleName} ({minWords}-{maxWords} words)");

        if (craftPosture is not null)
            sections.Add($"Craft Posture: {craftPosture}");

        if (narrativeVoice is not null && narrativeVoice.Count > 0)
        {
            var voiceLines = new List<string> { "Voice Textures:" };
        ...
        if (motifs is not null && motifs.Count > 0)
            sections.Add($"Motifs: {string.Join(", ", motifs)}");
